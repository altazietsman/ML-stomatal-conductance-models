...
df2 = df_anddereg[['Species','RH','Tair','Patm','PARin','SWC','Cond']]
df2 = df2.replace(-9999.0, np.nan)

df_org['PARin'] = df_org['solar']*0.45*4.57

df1 = df_org[['Species','RH','Tair','Patm','PARin','SWC','Cond']]
...
#pairplot(df)
#plt.show() #temp and solar is colinear so we can remove 1
...
y = df['Cond']
x = df.drop(['Cond','Tair','z_score'], axis = 1)
...
x_train, x_test, y_train, y_test = (train_test_split(x_standard,y, test_size = 0.3, random_state = 50))
